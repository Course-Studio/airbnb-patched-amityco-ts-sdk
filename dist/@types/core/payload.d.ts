...
declare global {
    namespace Amity {
         ...
         */
        type ProcessedChannelPayload<T extends Amity.ChannelType = any> = Omit<ChannelPayload, 'channels' | 'channelUsers' | 'messagePreviews' | 'messageFeedsInfo' | 'users'> & {
        ...
        type MessagePayload<T extends Amity.MessageContentType = any> = Omit<ProcessedMessagePayload<T>, 'messages' | 'messageFeeds'> & {
            messages: Amity.RawMessage<T>[];
            messageFeeds?: Amity.RawSubChannel[];
        };
        type StreamPayload = {
        ...
        type GlobalStoryFeedPayload = Omit<Amity.StoryBasePayload, 'comments'> & Amity.StoryTargetPayload;
        ...
        type ProcessedNotificationTrayPayload = Omit<NotificationTrayPayload, 'notificationTrayItems' | 'users'> & {
        ...
        type ProcessedCommentPayload<T extends Amity.CommentContentType = any> = Omit<CommentPayload<T>, 'users' | 'files' | 'communityUsers'>;
        type PollPayload = {
            users: Amity.InternalUser[];
            polls: Amity.Poll[];
        };
