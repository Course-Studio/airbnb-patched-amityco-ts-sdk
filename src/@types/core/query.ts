  ...
  namespace Amity {
    ...
    type Snapshot<T> = (T extends Amity.Cached<infer T2>
      ? T2 extends Amity.Paged<infer T3, infer T4>
        ? { data: T3[] | undefined } & Amity.Pages<T4> & { paging?: Amity.Pagination['paging'] } & {
              cachedAt?: Amity.CacheOptions['cachedAt'];
            }
        : { data: T2 } & { cachedAt?: Amity.CacheOptions['cachedAt'] }
      : T extends Amity.Paged<infer T3, infer T4>
      ? { data: T3[] | undefined } & Amity.Pages<T4> & { paging?: Amity.Pagination['paging'] } // paging is not existed in some collections
      : { data: T | undefined }) &
      SnapshotOptions;
