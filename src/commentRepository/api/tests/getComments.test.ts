...
describe('getCommentByIds', () => {
  ...
  test('Should return the comments from backend', async () => {
    ...
    expect(data).toEqual(commentsListResponse.data.map(comment => LinkedObject.comment(comment)));
  });
  ...
  test('Should return comments from locally', () => {
    ...
    expect(comments?.data).toEqual(
      commentsListResponse.data.map(comment => LinkedObject.comment(comment)),
    );
  });
